...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simSwitchDronesLight", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->switchDronesLight(light_name);
        });

        pimpl_->server.bind("simSetLightColor1", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->setLightColor1(light_name);
        });

        pimpl_->server.bind("simSetLightColor2", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->setLightColor2(light_name);
        });

        pimpl_->server.bind("simSetLightColor3", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->setLightColor3(light_name);
        });

        pimpl_->server.bind("simSetLightColor4", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->setLightColor4(light_name);
        });

        pimpl_->server.bind("simBatteryDead", [&](const std::string& light_name) -> bool {
            return getWorldSimApi()->setBatteryDead(light_name);
        });
