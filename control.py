...
class Control:
    def __init__(self, cfg: Config, step_size: float, no_backend: bool = False) -> None:
        self.cfg = cfg
    ...
    def moveToPosition(self, poses, sleep = 0.0):
        for dispathcer, drone_names in self.cfg.dispatchers.items():
            for drone in drone_names[::-1]:
...
if __name__ == '__main__':
    ...
    if ALLOW_MAPF:
        ...
        for i in tqdm(range(1000000)):
            ...
            if i % mapf.STEPS_PER_DISPATCH == 0:
                all_drones = main_control.cfg.all_drones
                for idx in mapf.allowed - allowed:
                    main_control.client.simSwitchDronesLight(all_drones[idx].name)
                allowed = mapf.allowed
            if code == 0:
                break
            main_control.moveToPosition(mdrones.position)
    else:
        main_control.moveToPosition(poses, 0.1)
        for drone in main_control.cfg.all_drones:
                main_control.client.simSwitchDronesLight(drone.name)
    ...
    if Flag_failures:
        ...
        for _ in range(FailureHandling.SIM_LIGHT_FAILURES):
            ...
            main_control.client.simSwitchDronesLight(standby_drone.name)
            main_control.client.simSetLightColor4(standby_drone.name)
            main_control.client.simSetLightColor3(light_failed_drone.name)
        ...
        for drone in ids:
            main_control.client.simSwitchDronesLight(main_control.cfg.all_drones[drone].name)
